...
export type StudioCopy = {
  ...
  ui: {
    ...
    selectedNode: string;
    canvasGuide: string;
    /** Shown while the walkthrough is being watched rather than driven. */
    canvasWatch: string;
    /** The read-only field under a watched walkthrough, in its three states. */
    composeWatch: string;
...
const zh: StudioCopy = {
  ...
  options: {
    s0a: "我自己就在用 TradingView。身边还有写不出代码的，和白天上班盯不了盘的。",
    s0b: "说实话我只想到「散户」，还没分得更细。",
    s1a: "就说我自己吧。规则在 TradingView 里写好了，可白天要上班。",
    s1b: "会写一点代码、只是想省事的那种。",
    s2a: "上周二开盘我在开会。到了我设的价位没人下单，看到手机时已经跑掉了。",
    s2b: "规则我记得住，但执行不稳定，情绪一上来就改了。",
    s3a: "不用更好的策略。把我定好的规则执行掉就行。",
    s3b: "帮我盯着，条件到了先问我一句，别自己乱动。",
    s4a: "读懂我的规则，动手前说一句要做什么，先在模拟里跑，我随时能停。",
    s4b: "先只做提醒，别自己下单。",
    sha: "等一下。真让它下单我还是不放心，第一版先只跑模拟吧。",
    shb: "反过来说，我更在意随时能按停，并且知道停在哪。",
    s5a: "先聊聊到底谁会用。",
    s5b: "那第一步，先让我的规则完整跑一次模拟。",
  ...
  replies: {
    ...
    s3a: ["那它具体要替你做哪几件事？"],
    s3b: ["那它具体要替你做哪几件事？"],
    s4a: ["现在整体有形状了。", "不过「可能会用的人」还是三个猜测。可以在地图里选中它深入，也可以先说第一步。"],
    s4b: ["那模拟执行先留着，等你放心再打开。", "「可能会用的人」还是三个猜测。可以在地图里选中它深入，也可以先说第一步。"],
    sha: ["那就把「执行前先讲清楚要做什么」排到第一位。", "真实下单不进第一版，先在模拟里跑通。"],
    shb: ["那就把「随时能停，并说明停在哪一步」排到第一位。", "真实下单同样不进第一版。"],
    s5b: ["记下了。算做对了的标准是：这一次模拟从头跑完，每一步都说得清为什么，而且随时能停下。"],
  },
  branches: {
    people: {
...
const en: StudioCopy = {
  ...
  ui: {
    ...
    canvasWatch: "The product shape, growing out of the conversation",
  ...
  options: {
    s0a: "Me, for a start — my rules already live in TradingView. Plus friends who can't code, and ones at work all day.",
    s0b: "Honestly, just “retail investors”. I haven't split it up yet.",
    s1a: "Take me: the rules are written in TradingView, but I'm at work when the market opens.",
    s1b: "The ones who can code a little and just want to save the trouble.",
    s2a: "Last Tuesday I was in a meeting at the open. My price hit, nobody acted, and it had moved when I looked.",
    s2b: "I remember the rule fine, but I don't follow it — emotion takes over.",
    s3a: "Not a better strategy. Just run the rule I already decided on.",
    s3b: "Watch it for me, and ask me first when the condition hits. Don't act on its own.",
    s4a: "Read my rule, say what it's about to do, run it in simulation, and stop when I say.",
    s4b: "Just alert me for now. Don't place orders on its own.",
    sha: "Hold on, though. Real orders still worry me — version one should stay in simulation.",
    shb: "The other way round: what matters most is stopping it, and seeing where it stopped.",
    s5a: "Let's talk about who would actually use it.",
    s5b: "So the first step: I run my own rule through one full simulation.",
  ...
  replies: {
    ...
    s4a: ["The whole thing has a shape now.", "“People who might use it” is still three guesses — select it on the map to go deeper, or name the first step."],
    s4b: ["Then simulated execution stays parked until you want it.", "“People who might use it” is still three guesses — select it on the map to go deeper, or name the first step."],
    sha: ["Then explaining before acting moves to the front.", "Real orders stay out of version one; it runs in simulation first."],
    shb: ["Then stopping it moves to the front.", "Real orders stay out of version one either way."],
    s5b: ["Noted. Done means that one simulation runs end to end, every step says why, and you can stop it at any point."],
  },
  branches: {
    people: {
...
const ja: StudioCopy = {
  ...
  ui: {
    ...
    canvasWatch: "会話に沿って育っていくプロダクトの輪郭",
    composeWatch: "読み取り専用 · 入力は再現",
    composeTyping: "入力を再現中…",
    composeSending: "送信します",
  ...
  options: {
    s0a: "まず自分です。ルールは TradingView に書いてあります。あとはコードが書けない人と、日中は仕事の人。",
    s0b: "正直まだ「個人投資家」としか考えていません。",
    s1a: "自分の場合ですね。ルールは TradingView にあるのに、日中は仕事なんです。",
    s1b: "少しコードが書けて、手間を減らしたいだけの人です。",
    s2a: "先週の火曜、寄り付きは会議中でした。指値に届いても誰も出さず、スマホを見たら戻っていました。",
    s2b: "ルールは覚えているのに執行がぶれます。感情が入ると変えてしまう。",
    s3a: "もっと良い戦略はいりません。決めたルールを執行してくれれば十分です。",
    s3b: "見張っておいて、条件が来たら先に一声かけてほしい。勝手には動かないで。",
    s4a: "ルールを読んで、動く前に何をするか言って、まず模擬で走らせて、いつでも止められること。",
    s4b: "まずは通知だけで十分です。自分で発注はしないでほしい。",
    sha: "いや、待ってください。本当に発注させるのはまだ怖い。初版は模擬実行だけにします。",
    shb: "逆に、いつでも止められて、どこで止まったか分かる方が大事です。",
    s5a: "まず、誰が使うのかを話したい。",
    s5b: "では最初の一歩は、自分のルールを一度最後まで模擬実行することで。",
  ...
  replies: {
    ...
    s4a: ["全体の形が見えてきました。", "ただ「使いそうな人」はまだ三つの推測です。マップで選んで深掘りしても、先に最初の一歩でも構いません。"],
    s4b: ["では模擬実行は置いておいて、安心できたら開けましょう。", "「使いそうな人」はまだ三つの推測です。マップで選んで深掘りしても、先に最初の一歩でも構いません。"],
    sha: ["では「実行の前に何をするか説明する」を先頭に置きます。", "実資金の発注は初版に入れません。まず模擬実行で通します。"],
    shb: ["では「いつでも止まり、どこで止まったか示す」を先頭に置きます。", "実資金の発注は、どちらにしても初版には入れません。"],
    s5b: ["メモしました。できたと言える条件は、その模擬実行が最後まで通り、各ステップの理由が分かり、いつでも止められることです。"],
  },
  branches: {
    people: {
