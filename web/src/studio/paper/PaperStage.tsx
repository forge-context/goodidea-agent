...
import {
  INK, LAYOUT, MOBILE_STAGE, ROLES, STAGE, geo,
  type AskState, type Role, type Scene,
} from "./paperTimeline";
import type { PaperCopyShape } from "./paperCopy";
import { ASSETS, Designer, Desk, Ink, Paper, ScopeSketch } from "./stageParts";
  ...
  return (
    <div className="gip-workspace" style={{ opacity: w.opacity }}>
      <div className="gip-ws-bar">
        <div className="gip-ws-brand"><span className="gip-ws-symbol" aria-hidden="true">g</span><strong>GoodIdea</strong></div>
      ...
      <div className="gip-ws-foot">
        ...
        <span className="gip-ws-decisions">{copy.workspace.decisions[w.decision]}</span>
      </div>
    </div>
  );
}
 ...
 *
 * Each agent shows at most two lines: what it is doing now, and what it has handed in.
 * Neither line moves unless the film has actually shown that change, so nothing here
 * spins, blinks, or piles up a log. The bottom line is GoodIdea's own state — how many
 * contributions are in, and how many decisions that turned into.
 */
 ...
 * about are both stated, and only then does one decision come out of it. Without this
 * the film would jump from three arrows straight to a proposal, which is the part a
 * viewer would have to take on trust.
 */
function FusionPanel({ scene, copy }: Props) {
  const f = scene.fusion;
  if (f.opacity <= 0.001) return null;
  const g = geo(scene.compact).fusion;
  const c = copy.fusion;
  return (
    <div className="gip-fusion" style={{ left: g.x, top: g.y, width: g.w, minHeight: g.h, opacity: f.opacity }}>
      <p className="gip-fusion-kicker"><span className="gip-fusion-dot" aria-hidden="true" />{c.kicker}</p>
...
function OutlookCard({ scene, copy }: Props) {
  ...
  return (
    <>
      ...
      <div className="gip-line gip-asks" style={row(P.rows.asks, 1)}>
        {copy.opening.questions.map((q, i) => (
          <div key={q} className={`gip-ask${askClass(ol.asks[i])}`} style={{ opacity: ol.asks[i].opacity }}>{q}</div>
        ))}
      </div>
      <div className="gip-line gip-preview" style={row(P.rows.preview, 1)}>
        ...
        <div className="gip-preview-foot" style={{ opacity: ol.button === "hidden" ? 0 : 1 }}>
          <span className={`gip-btn is-client is-${ol.button}`}>{ol.button === "done" ? c.buttonDone : c.button}</span>
...
function ProductPaper({ scene, copy }: Props) {
  ...
  return (
    <Paper
      pose={scene.product} w={P.w} h={P.h} src={`${ASSETS}/product-paper.webp`}
    ...
    >
      <div className="gip-sheet" style={{ left: P.pad.left, right: P.pad.right }}>
        ...
        <div className="gip-line gip-original" style={row(P.rows.original1, tx.originalLines[0] * tx.original)}>{c.original[0]}</div>
        <div className="gip-line gip-original" style={row(P.rows.original2, tx.originalLines[1] * tx.original)}>{c.original[1]}</div>
        <div className="gip-line gip-revised" style={row(lerp(P.rows.revised1, P.rows.liftTo1, tx.lift), tx.revised1)}>{c.revised[0]}</div>
        <div className="gip-line gip-revised" style={row(lerp(P.rows.revised2, P.rows.liftTo2, tx.lift), tx.revised2)}>{c.revised[1]}</div>
        {/* The proposal keeps its own dashed edge and is put away once answered: it
            never turns into product content on its own. */}
        <div className="gip-line gip-proposal" style={row(P.rows.proposalCard, tx.proposal * scene.proposalOpacity)}>
          ...
          <div className="gip-proposal-detail" style={{ opacity: tx.proposalDetail }}>
            {c.proposalDetail.map((line) => <div key={line}>{line}</div>)}
          </div>
        ...
        <div className="gip-line gip-button-row" style={row(scene.compact ? P.rows.buttonCompact : P.rows.button, scene.button === "hidden" ? 0 : scene.proposalOpacity)}>
          <span className={`gip-btn is-${scene.button}`}>{done ? c.buttonDone : c.button}</span>
        </div>
        ...
        {ol.card > 0.001 && <div className="gip-outlook" style={{ opacity: ol.card }}><OutlookCard scene={scene} copy={copy} /></div>}
      </div>
      <svg className="gip-paper-ink" viewBox={`0 0 ${P.w} ${P.h}`} style={{ opacity: tx.original }} aria-hidden="true">
        <Ink strokes={INK.strike} progress={scene.ink.strike} className="gip-ink gip-ink-strike" />
      </svg>
    </Paper>
  );
}

function WorkPaper({ scene, copy, role }: Props & { role: Role }) {
      ...
      <Paper pose={pose} w={W.w} h={W.h} src={SHEET[role]}>
        ...
        <div className="gip-sheet" style={{ left: W.pad.left, right: W.pad.right }}>
          ...
          {role === "research" && copy.work.researchLines.map((line, i) => (
            <div key={line} className="gip-line gip-work-note" style={row(W.rows.researchLine + i * W.rows.researchGap, tx.lines[i])}>{line}</div>
          ))}
        ...
        <svg className="gip-paper-ink" viewBox={`0 0 ${W.w} ${W.h}`} aria-hidden="true">
          {role === "research" && <Ink strokes={INK.underline} progress={scene.ink.underline} className="gip-ink gip-ink-underline" />}
          {role === "engineering" && <Ink strokes={INK.engineering} progress={scene.ink.engineering} className="gip-ink gip-ink-engineering" />}
          {/* Engineering's circle stays on UX's sheet, so it is still there when every
              sheet has been parked side by side. */}
          {role === "experience" && <Ink strokes={INK.scopeCircle} progress={scene.ink.scopeCircle} className="gip-ink gip-ink-circle" width={2.4} />}
        </svg>
      </Paper>
    </div>
  );
...
function Cursor({ scene }: { scene: Scene }) {
  const { visible, x, y, pressed } = scene.cursor;
  if (!visible) return null;
  return (
    <div className={`gip-cursor${pressed ? " is-pressed" : ""}`} style={{ transform: `translate(${x}px, ${y}px)` }}>
  ...
  return (
    ...
    >
      ...
      <div className="gip-scene" style={{ opacity: scene.fade }} aria-hidden="true">
        ...
        <WorkPaper scene={scene} copy={copy} role="research" />
        <WorkPaper scene={scene} copy={copy} role="experience" />
        <WorkPaper scene={scene} copy={copy} role="engineering" />
        <ParkedTags scene={scene} copy={copy} />
        <FusionPanel scene={scene} copy={copy} />
        <ProductPaper scene={scene} copy={copy} />
        {scene.ink.arrowOpacity > 0.001 && (
          <svg className="gip-stage-ink" viewBox={`0 0 ${stage.w} ${stage.h}`} style={{ opacity: scene.ink.arrowOpacity }} aria-hidden="true">
            ...
            <Ink
              strokes={scene.compact ? INK.mobileFusionArrow : INK.fusionArrow}
        ...
        {b0 > 0.001 && (
          <div className="gip-brand" style={{ left: g.brand.x, top: g.brand.y }}>
            <div className="gip-brand-mark" style={{ opacity: b0 }}>{copy.brand.mark}</div>
