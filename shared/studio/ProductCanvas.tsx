...
import type { CanvasCopy, CanvasView, Placement } from "./types";
...
function tidy<B extends string>(
  nodes: Placement<B>[],
  heights: Record<string, number>,
  boardWidth: number,
): Laid<B>[] {
  const items: Laid<B>[] = nodes
    .map((node) => {
      const width = Math.min(
        (node.w / 100) * boardWidth,
        boardWidth - 2 * EDGE_PAD,
      );
      const half = width / 2;
      return {
        node,
        ...
        centre: Math.min(
          Math.max((node.x / 100) * boardWidth, half + EDGE_PAD),
          boardWidth - half - EDGE_PAD,
        ),
        width,
        top: node.y,
...
export function ProductCanvas<B extends string>({
  view,
  copy,
  dim,
  selectedNodeId,
  ...
  onSelectNode?: (node: Placement<B>) => void;
  ...
  const shapeKey = view.nodes
    .map((node) => `${node.id}:${node.content}:${node.w}`)
    .join("|");
  ...
  useLayoutEffect(() => {
    ...
    if (changed || Object.keys(next).length !== Object.keys(heights).length)
      setHeights(next);
  }, [shapeKey, box.width, view.nodes, heights, copy.nodes, textOverrides]);

  const laid = useMemo(
    () => tidy(view.nodes, heights, box.width || 360),
    [view.nodes, heights, box.width],
  );
  const contentHeight =
    laid.reduce(
      (tallest, item) => Math.max(tallest, item.top + item.height),
      0,
    ) + 4;
  const scale =
    box.height > 0 && contentHeight > box.height
      ? Math.max(minScale, box.height / contentHeight)
      : 1;
  const lift =
    box.height > contentHeight
      ? Math.min((box.height - contentHeight) / 2, 120)
      : 0;
  ...
  useEffect(() => {
    ...
    const middle =
      (targets.reduce((sum, item) => sum + item.top + item.height / 2, 0) /
        targets.length) *
      scale;
    element.scrollTo({
      ...
      >
        ...
        >
          {view.edges.map((edge) => {
            ...
            const selectedLit =
              !selectedRelated ||
              selectedRelated.has(edge.from) ||
              selectedRelated.has(edge.to);
            const lit =
              selectedLit &&
              (dim !== "strong" || focus.has(edge.from) || focus.has(edge.to));
            return (
              <line
                key={edge.id}
...
function CanvasNodeView<B extends string>({
  item,
  boardWidth,
  copy,
  dim,
  ...
  const left = settled
    ? item.centre
    : ((node.from?.x ?? node.x) / 100) * boardWidth;
...
function NodeBody({
  ...
  text: { caption?: string; text: string; detail?: string[]; note?: string };
}) {
  return (
    <>
      {text.caption && (
        <span className="canvas-node-caption">{text.caption}</span>
      )}
